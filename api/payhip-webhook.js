...
import { json, readJsonBody, sha256Hex, env, supabaseFetch, sendEmail, addHoursIso } from "./_utils.js";
...
export default async function handler(req, res){
  try{
    ...
    if (requiredProductId && productId && requiredProductId !== productId){
      // Ignore purchases for other products
      return json(res, 200, { ok: true, ignored: true, reason: "wrong_product" });
    }

    // 3) Idempotency: if purchase exists, do nothing (but return 200)
    const existing = await supabaseFetch(`/rest/v1/purchases?payhip_transaction_id=eq.${encodeURIComponent(transactionId)}&select=id`, { method: "GET" });
    if (Array.isArray(existing) && existing.length){
      return json(res, 200, { ok: true, duplicate: true });
    }

    // 4) Upsert customer (grant access)
    // Upsert via PostgREST: insert with Prefer: resolution=merge-duplicates using unique constraint on email
    ...
    const link = `${baseUrl.replace(/\/$/,"")}/magic.html?token=${token}`;
